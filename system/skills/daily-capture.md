...
> **CANONICAL — curator-only.** This contract is owned by the vault owner (and their designated curator). All other agents — including any tool-specific skill — are **readers**: follow this note, and if you believe it should change, propose the change rather than making it. **Do not edit this file directly.** Direct rewrites by non-curator agents have repeatedly introduced drift (e.g. silently relocating the active daily note out of the vault root) and will be reverted.
